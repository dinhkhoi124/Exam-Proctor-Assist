  ...
  return (
    <div className="flex h-screen w-full bg-slate-50 overflow-hidden">
      ...
      <main className="flex-1 flex flex-col h-full overflow-hidden">
        ...
        <nav className="flex shrink-0 gap-2 overflow-x-auto border-b bg-white px-3 py-2 md:hidden">
          <Link
            to="/"
            className="flex shrink-0 items-center gap-1.5 rounded-lg border px-3 py-2 text-xs font-semibold text-slate-600"
          >
        ...
        {/* SCROLLABLE OUTLET */}
        <div className="flex-1 overflow-y-auto overflow-x-hidden bg-slate-50 p-3 sm:p-4 md:p-8">
          <Outlet />
        </div>
      </main>
      ...
      {showToggleModal && pendingViewMode && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/50 p-4 backdrop-blur-xs animate-in fade-in duration-200">
          <div className="max-h-[90dvh] w-full max-w-sm overflow-y-auto rounded-xl border bg-white p-5 shadow-lg sm:p-6 space-y-4 animate-in zoom-in-95 duration-200 text-left">

...
export default function Reports() {
  ...
  const loadPreview = async () => {
    ...
    setIsLoading(true);
    try {
      const response = await api.get<ReportPreview>(`/api/v1/admin/reports/preview?${params()}`);
  ...
  const exportReport = async (format: ExportFormat) => {
    ...
    try {
      ...
      link.href = url;
      link.download = `chatbot-report-${startDate}-${endDate}.${format}`;
  ...
  return (
    <div className="space-y-6 pb-10">
      ...
      <div className="rounded-xl border bg-white p-5 shadow-sm">
        <div className="grid gap-4 md:grid-cols-4">
          ...
          <div className="flex items-end">
            <Button onClick={loadPreview} disabled={isLoading} className="w-full bg-orange-600 hover:bg-orange-700">
        ...
        <div className="mt-5 flex flex-wrap gap-3 border-t pt-5">
          <Button variant="outline" onClick={() => exportReport("xlsx")} disabled={exporting !== null}>
            {exporting === "xlsx" ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <FileSpreadsheet className="mr-2 h-4 w-4 text-emerald-600" />}
            Xuất Excel
          </Button>
          <Button variant="outline" onClick={() => exportReport("pdf")} disabled={exporting !== null}>
      ...
      {isLoading && !preview ? (
        ...
        <>
          <div className="grid gap-4 md:grid-cols-4">
            <SummaryCard label="Tổng câu hỏi" value={preview.summary.total_questions.toLocaleString()} icon={<BarChart3 />} />

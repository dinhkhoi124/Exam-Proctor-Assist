...
def _date_bounds(start_date: date, end_date: date) -> tuple[datetime, datetime]:
    return (
        datetime.combine(start_date, time.min, tzinfo=VN_TZ).astimezone(timezone.utc),
        datetime.combine(end_date + timedelta(days=1), time.min, tzinfo=VN_TZ).astimezone(timezone.utc),
    )
...
def _append_table(workbook: Workbook, title: str, headers: list[str], rows: list[list[Any]]):
